...
const setupTheme = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "deeprootSite"))) {
    ...
    const folder = createNewFolder(rootFolder, "deeprootSite");

    fs.readdirSync(rootFolder, { withFileTypes: true }).forEach((file) => {
      if (includesFiles.includes(file.name)) {
        if (file.isDirectory()) {
          const destination = path.join(rootFolder, "deeprootSite", file.name);

...
const getFolderName = (rootFolder) => {
  const configPath = path.join(rootFolder, "deeprootSite/hugo.toml");
...
const setupProject = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "themes"))) {
    ...
    [
      {
        filepath: path.join(rootFolder, "deeprootSite/hugo.toml"),
        regex: /^.*theme\s*=\s*("[^"\]]+"|\S+)/m,
      },
      {
        filepath: path.join(
          rootFolder,
          "deeprootSite/config/_default/module.toml",
        ),
        regex: /\[\[imports\]\]\s*\r?\npath = "([^"]+)"/,
      },
    ...
    const deeprootSite = path.join(rootFolder, "deeprootSite");
    iterateFilesAndFolders(deeprootSite, { destinationRoot: rootFolder });
    deleteFolder(deeprootSite);
  } else {

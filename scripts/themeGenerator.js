...
function determinePaths() {
  ...
  const exampleSiteHugoToml = path.join(__dirname, "../deeprootSite/hugo.toml");

  if (fs.existsSync(exampleSiteHugoToml)) {
    // Theme setup mode - deeprootSite structure exists
    return {
      hugoTomlPath: exampleSiteHugoToml,
      themePath: path.join(__dirname, "../deeprootSite/data/theme.json"),
    ...
    throw new Error(
      "Could not determine setup mode: neither deeprootSite/hugo.toml nor root hugo.toml found",
    );
  }
}
